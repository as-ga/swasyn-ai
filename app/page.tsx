...
export default function Home() {
  ...
  const [reportData, setReportData] = useState<SimplifyMedicalJargonOutput | null>(null);
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([]);
  const [isProcessingReport, startProcessReportTransition] = useTransition();
  const [reportText, setReportText] = useState("");
  const [originalReportData, setOriginalReportData] = useState<SimplifyMedicalJargonOutput | null>(null);
  const UploadCard = dynamic(() => import("@/components/swasyn-ai/UploadCard"), {
    ssr: false,
  });
